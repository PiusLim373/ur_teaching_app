...
class MoveGroupPythonInteface(object):
    ...
    def __init__(self):
        ...
        sb_gripper_manipulator = moveit_commander.MoveGroupCommander("sb_gripper_manipulator")
        locker_gripper_manipulator = moveit_commander.MoveGroupCommander("locker_gripper_manipulator")
                ...
                for i in range(num_point):
                    if i > 0:
                        if path.joint_trajectory.points[i].positions[1] >= -0.2:
                            large_turning = True
                            print("new plan cartesian debug", str(path.joint_trajectory.points[i].positions[1]), str(i))
                    ...
                    if i > 0:
                        ...
                        if ((check_joint_0 > 1.745) or (check_joint_1 >= -0.2) or (check_joint_2 <= 0) or (check_joint_3 > 0.3)):
                            ...
                            if (check_joint_0 > 1.745):
                                rospy.logerr("check_joint_0 large turning error caught" + str(check_joint_0))
                            if (check_joint_1 >= -0.2):
                                rospy.logerr("check_joint_1 large turning error caught" + str(check_joint_1))
                            if (check_joint_2 <= 0):
                                rospy.logerr("check_joint_2 large turning error caught" + str(check_joint_2))
                            if (check_joint_3 > 0.3):
                                rospy.logerr("check_joint_3 large turning error caught" + str(check_joint_3))
                            break
                if not large_turning:
                    speedup_path = self.speedup(path)
...
class MotionServer():
    ...
    def execute(self, goal):
        ...
        ########################################################################## motion server: sparebay homing #####################33
            ...
            result.success = control.plan_and_move(eval(goal.payload), ignore_large_angle_checking = True)
        ########################################################################## motion server: sparebay homing #####################33
        ...
        ########################################################################## motion server: sparebay homing #####################33
            ...
            result.success=control.plan_and_move(pose, ignore_large_angle_checking = True)
        ########################################################################## motion server: sparebay homing #####################33
...
class DashboardServer():
    ...
    def dashboard_task(self, action):
        if action == "init":
            ...
            if self.dashboard_command("safetystatus") == "ROBOT_EMERGENCY_STOP": #with estop depressed, normal if wihtout estop depressed
                rospy.logerr("ESTOP need to be unpressed before activating robot")
                return False
            elif self.dashboard_command("safetystatus") == "PROTECTIVE_STOP":
                rospy.loginfo("Robot reached a protective stop, waiting for 5 sec before releasing")
                sleep(5)
