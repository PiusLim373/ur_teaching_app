...
export default function CustomJoint(props) {
  ...
  const verifyPayload = (event) => {
    ...
    try {
      let joint_list = JSON.parse("[" + event.target.value + "]");
      if (joint_list.length === 6) {
        setPayloadVerified(true);
        setPayloadValue(joint_list);
      } else {
    ...
    axios
      ...
      .then((res) => {
        if (res.data) {
          let event = {
            target: {
              value: JSON.stringify(res.data.current_joint).replace(
                /[\[\]']+/g,
                ""
              ),
            },
  ...
  return (
    <React.Fragment>
      ...
      <InputGroup>
        ...
        <Form.Control
          placeholder="List of 6 joints, seperated by comma"
